...
public class LeviathanEnemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //projectileEmittersControllerRef = GetComponentsInChildren<ProjectileEmittersController>();

        // --- BUILD COMBINED GUN ARRAY ---
        List<Transform> gunList = new List<Transform>();
        foreach (var em in projectileEmittersControllerRef)
            gunList.AddRange(em.GetGunsArray());
        gunsPositions = gunList.ToArray();

        // weapon timing based on FIRST emitter (or averages if you prefer)
        if (projectileEmittersControllerRef.Length > 0)
        {
            var em = projectileEmittersControllerRef[0];

            weaponShootDuration = em.GetSequenceDuration() + 0.1f;
